/**
 * RFC 016：按 origin 覆盖主题滤镜（部分字段）；与全局合并后得到有效配置。
 */
...
export interface SiteOverrideEntryV1 {
  /** @deprecated 仅 Dynamic；旧 storage 可能仍有此字段，解析时忽略。 */
  themeMode?: ThemeMode
...
/**
 * 合并全局与单站覆盖，得到注入用有效主题滤镜。
 */
export function resolveEffectiveTheme(
  origin: string,
  _globalMode: ThemeMode,
  ...
  return { themeMode: THEME_MODE_DYNAMIC, themeFilters }
}

/**
 * 合并全局与单站的网页配色（Dynamic 路径）。
 */
